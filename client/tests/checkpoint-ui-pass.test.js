...
assert.ok(app.includes("const rewardItems = createRewardRows(reward, { compact:true })"));
assert.ok(app.includes("prize.appendChild(rewardItems)"));
...
assert.ok(app.includes('giftAction.textContent = remaining > 0 ? formatChestCountdown(remaining) : "HEDİYE SANDIK AÇ"'));
...
assert.ok(html.includes('data-team-panel="profile" class="team-panel team-profile-panel is-active"'));
assert.ok(html.includes('id="team-profile-total-trophies"'));
assert.ok(html.includes('id="team-profile-tournament-points"'));
...
assert.ok(html.includes('data-screen-panel="weekly-event"'));
assert.ok(html.includes('data-screen-panel="team-event"'));
assert.ok(html.includes('data-weekly-event-tab="standings"'));
assert.ok(html.includes('data-team-event-tab="fixtures"'));
assert.ok(html.includes('id="team-profile-dialog"'));
...
assert.ok(html.includes('id="daily-meta-dialog"'));
assert.ok(html.includes('id="daily-meta-roll"'));
assert.ok(html.includes('id="daily-meta-card"'));
assert.ok(app.includes('function loadDailyMetaState'));
assert.ok(app.includes('function rollDailyMeta'));
assert.ok(app.includes('activeTeamTab = "profile"'));
assert.ok(app.includes('/daily-meta/roll'));
assert.match(css, /\.daily-meta-wheel\.is-spinning/);
