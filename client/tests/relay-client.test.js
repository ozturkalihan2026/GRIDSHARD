...

{
  ...
  assert.ok(src.includes("RC aday tarayıcı durumu aktif"));
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("RC aday tarayıcı durumu aktif")); // alpha32->33 protocol status
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("RC aday tarayıcı durumu aktif")); // alpha33 protocol
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("RC aday tarayıcı durumu aktif")); // alpha34 websocket
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("RC aday tarayıcı durumu aktif")); // alpha35 gateway
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("RC aday tarayıcı durumu aktif")); // alpha36 setup
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("RC aday tarayıcı durumu aktif")); // alpha37 lobby
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("RC aday tarayıcı durumu aktif")); // alpha38 runner
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("RC aday tarayıcı durumu aktif")); // alpha39 heartbeat
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("RC aday tarayıcı durumu aktif")); // alpha40 online pvp
}
...
Promise.all(asyncTests).then(() => {
  console.log("131 client tests passed");
}).catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
