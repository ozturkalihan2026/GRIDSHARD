...

{
  ...
  assert.ok(src.includes("Go/No-Go teknik durum görünümü aktif"));
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go teknik durum görünümü aktif")); // alpha32->33 protocol status
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go teknik durum görünümü aktif")); // alpha33 protocol
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go teknik durum görünümü aktif")); // alpha34 websocket
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go teknik durum görünümü aktif")); // alpha35 gateway
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go teknik durum görünümü aktif")); // alpha36 setup
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go teknik durum görünümü aktif")); // alpha37 lobby
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go teknik durum görünümü aktif")); // alpha38 runner
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go teknik durum görünümü aktif")); // alpha39 heartbeat
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go teknik durum görünümü aktif")); // alpha40 online pvp
}
...
Promise.all(asyncTests).then(() => {
  console.log("125 client tests passed");
}).catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
