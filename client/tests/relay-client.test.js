...

{
  ...
  assert.ok(src.includes("Go/No-Go uçtan uca huni sinyalleri aktif"));
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go uçtan uca huni sinyalleri aktif")); // alpha32->33 protocol status
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go uçtan uca huni sinyalleri aktif")); // alpha33 protocol
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go uçtan uca huni sinyalleri aktif")); // alpha34 websocket
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go uçtan uca huni sinyalleri aktif")); // alpha35 gateway
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go uçtan uca huni sinyalleri aktif")); // alpha36 setup
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go uçtan uca huni sinyalleri aktif")); // alpha37 lobby
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go uçtan uca huni sinyalleri aktif")); // alpha38 runner
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go uçtan uca huni sinyalleri aktif")); // alpha39 heartbeat
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Go/No-Go uçtan uca huni sinyalleri aktif")); // alpha40 online pvp
}
...
Promise.all(asyncTests).then(() => {
  console.log("126 client tests passed");
}).catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
