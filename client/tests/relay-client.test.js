...

{
  ...
  assert.ok(src.includes("Gerçek Web test operasyon izleme özeti hazır"));
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Gerçek Web test operasyon izleme özeti hazır")); // alpha32->33 protocol status
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Gerçek Web test operasyon izleme özeti hazır")); // alpha33 protocol
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Gerçek Web test operasyon izleme özeti hazır")); // alpha34 websocket
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Gerçek Web test operasyon izleme özeti hazır")); // alpha35 gateway
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Gerçek Web test operasyon izleme özeti hazır")); // alpha36 setup
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Gerçek Web test operasyon izleme özeti hazır")); // alpha37 lobby
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Gerçek Web test operasyon izleme özeti hazır")); // alpha38 runner
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Gerçek Web test operasyon izleme özeti hazır")); // alpha39 heartbeat
}

{
  const fs=require("fs");
  const src=fs.readFileSync("./src/app.js","utf8");
  assert.ok(src.includes("Gerçek Web test operasyon izleme özeti hazır")); // alpha40 online pvp
}
...
Promise.all(asyncTests).then(() => {
  console.log("144 client tests passed");
}).catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
