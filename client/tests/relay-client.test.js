...

{
  ...
  assert.ok(src.includes("GRIDSHARD Beta.17 · Engine Regresyonu + Audio Mix V2 + Hızlı Loadout"));
}

{
  const fs=require("fs");
  const src=fs.readFileSync(path.join(ROOT,"src/app.js"),"utf8");
  assert.ok(src.includes("GRIDSHARD Beta.17 · Engine Regresyonu + Audio Mix V2 + Hızlı Loadout")); // alpha32->33 protocol status
}

{
  const fs=require("fs");
  const src=fs.readFileSync(path.join(ROOT,"src/app.js"),"utf8");
  assert.ok(src.includes("GRIDSHARD Beta.17 · Engine Regresyonu + Audio Mix V2 + Hızlı Loadout")); // alpha33 protocol
}

{
  const fs=require("fs");
  const src=fs.readFileSync(path.join(ROOT,"src/app.js"),"utf8");
  assert.ok(src.includes("GRIDSHARD Beta.17 · Engine Regresyonu + Audio Mix V2 + Hızlı Loadout")); // alpha34 websocket
}

{
  const fs=require("fs");
  const src=fs.readFileSync(path.join(ROOT,"src/app.js"),"utf8");
  assert.ok(src.includes("GRIDSHARD Beta.17 · Engine Regresyonu + Audio Mix V2 + Hızlı Loadout")); // alpha35 gateway
}

{
  const fs=require("fs");
  const src=fs.readFileSync(path.join(ROOT,"src/app.js"),"utf8");
  assert.ok(src.includes("GRIDSHARD Beta.17 · Engine Regresyonu + Audio Mix V2 + Hızlı Loadout")); // alpha36 setup
}

{
  const fs=require("fs");
  const src=fs.readFileSync(path.join(ROOT,"src/app.js"),"utf8");
  assert.ok(src.includes("GRIDSHARD Beta.17 · Engine Regresyonu + Audio Mix V2 + Hızlı Loadout")); // alpha37 lobby
}

{
  const fs=require("fs");
  const src=fs.readFileSync(path.join(ROOT,"src/app.js"),"utf8");
  assert.ok(src.includes("GRIDSHARD Beta.17 · Engine Regresyonu + Audio Mix V2 + Hızlı Loadout")); // alpha38 runner
}

{
  const fs=require("fs");
  const src=fs.readFileSync(path.join(ROOT,"src/app.js"),"utf8");
  assert.ok(src.includes("GRIDSHARD Beta.17 · Engine Regresyonu + Audio Mix V2 + Hızlı Loadout")); // alpha39 heartbeat
}

{
  const fs=require("fs");
  const src=fs.readFileSync(path.join(ROOT,"src/app.js"),"utf8");
  assert.ok(src.includes("GRIDSHARD Beta.17 · Engine Regresyonu + Audio Mix V2 + Hızlı Loadout")); // alpha40 online pvp
}
