...
set "GRIDSHARD_EXPECTED_VERSION=2.0.0-beta.32-fix.1"
...
rem Boylece eski bir GRIDSHARD surumu 8000'de acik kalsa bile Beta.32 Fix.1 ona baglanmaz.
...
echo [GRIDSHARD] Beta.32 Fix.1 test portu: %GRIDSHARD_WEB_PORT%
start "GRIDSHARD Web Test Sunucusu" cmd /k call "%~dp0BASLAT_WEB_TEST.bat"

rem Tarayiciyi yalnizca secilen port gercekten Beta.32 Fix.1 /health yaniti verirse ac.
...
echo Tarayici dogrulanmis Beta.32 Fix.1 sunucusunda acildi: http://127.0.0.1:%GRIDSHARD_WEB_PORT%/
