...
def main():
    ...
    history=load(HISTORY,{'version':'2.0.0-beta.22','runs':[]})
    history['version']='2.0.0-beta.22'
    runs=history.setdefault('runs',[])
    if current.get('status')!='VERIFIED_PASSED' or current.get('verified_passed') is not True:
        COMPARISON.write_text(json.dumps({'version':'2.0.0-beta.22','status':'SKIPPED','reason':'Bütünlük doğrulamasını geçmiş gerçek Windows E2E importu yok.','history_count':len(runs),'current_added':False,'automatic_pass':False},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
