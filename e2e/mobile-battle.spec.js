...
test("dokunmatik savaş görünümü tek ekrana sığar ve seç-yerleştir çalışır", async ({ page }) => {
  ...
  for (let index = 0; index < 17; index += 1) {
    await page.locator("#battle-pool-selection .pool-choice-select[data-action=add]").first().click();
  }
  await page.locator("#battle-pool-confirm").click();

  await expect(page.locator("body")).toHaveAttribute("data-local-status", "battle", { timeout: 20_000 });
  ...
  await page.getByRole("button", { name: "Modül Rafı" }).click();
  ...
  await reserveCards.first().click();
...
test("ilk maç eğitimi hazır havuzu yükler ve AI savaşını başlatır", async ({ page }) => {
  await page.goto("/?e2e=tutorial", { waitUntil: "networkidle" });
  await page.getByRole("button", { name: "Oyna" }).click();
  ...
  await tutorial.getByRole("button", { name: "AI Savaşını Başlat" }).click();
  await expect(page.locator("body")).toHaveAttribute("data-local-status", "battle", { timeout: 20_000 });
