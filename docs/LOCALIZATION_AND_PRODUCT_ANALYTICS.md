...
- Profil, etkinlik, ödül, takım ve modül/çekirdek bilgi ekranları için durum etiketleri ve kozmetik/meta kimlikleri sabit katalog anahtarlarına bağlanır. Sunucu tarafından üretilen başlık/açıklama/effect alanları `*_tr` ve `*_en` olarak iletilir; istemci seçili dile göre alanı seçer. Yeni bir dil eklenirken metin düğümü taramasına veya Türkçe metin kalıbına bağlı yeni davranış eklenmemelidir.
- HTTP hata yanıtları `{ "code": "invalid_request", "detail": ... }` biçimindedir. `code` sabit, dil bağımsız protokol alanıdır; `detail` yalnız hata ayıklama/geriye dönük uyumluluk içindir. İstemci metni `error.<code>` anahtarından seçer; bilinmeyen kodlarda HTTP durumuna bağlı güvenli genel mesaja düşer. Yeni hata türü eklenirken sunucuda kod, istemcide TR/EN anahtar birlikte eklenmelidir. Savaş komutu reddi ve yerleştirme yetersiz Akım durumu da metin eşlemesiyle değil kodla işlenir.
